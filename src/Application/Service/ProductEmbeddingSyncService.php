...
class ProductEmbeddingSyncService
{
    ...
    public function generateEmbeddingText(Product $product): string
    {
        ...
        if (strlen($rawDescription) > self::MAX_RAW_DESCRIPTION_LENGTH) {
            throw new \InvalidArgumentException(sprintf('Product description is too long: %d characters (max: %d). Please reduce the description text before generating embeddings.', strlen($rawDescription), self::MAX_RAW_DESCRIPTION_LENGTH));
        }
    ...
    private function optimizeText(string $text): string
    {
        ...
        if (strlen($text) > self::MAX_DESCRIPTION_LENGTH) {
            ...
            if (false !== $lastPeriod && $lastPeriod > self::MAX_DESCRIPTION_LENGTH * 0.9) {
                $text = substr($text, 0, $lastPeriod + 1);
            } elseif (false !== $lastSpace && $lastSpace > self::MAX_DESCRIPTION_LENGTH * 0.9) {
