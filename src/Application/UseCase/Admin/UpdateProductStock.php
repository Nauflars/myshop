...
class UpdateProductStock
{
    ...
    public function execute(
        ...
        if (!in_array($mode, ['set', 'add', 'subtract'], true)) {
            throw new \InvalidArgumentException("Invalid mode '$mode'. Must be 'set', 'add', or 'subtract'");
        }
        ...
        if ($newStock < self::MIN_STOCK) {
            throw new \InvalidArgumentException("Stock cannot be negative (would result in: $newStock)");
        }

        if ($newStock > self::MAX_STOCK) {
            throw new \InvalidArgumentException('Stock exceeds maximum limit of '.self::MAX_STOCK);
        }
